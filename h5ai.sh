...
h5ai_zip="_h5ai.zip"
h5ai_download_http="https://zhujiwiki.com/usr/uploads/2018/03/_h5ai.zip"
...
server {
    ...
    server_name jaskon139-46589.portmap.io;
    root /home/wwwroot/jaskon139-46589.portmap.io;
...
server
    {
        listen 80;
        server_name jaskon139-46589.portmap.io;
        rewrite ^(.*) https://jaskon139-46589.portmap.io\$1 permanent;
    }
EOF
    if [[ $? -eq 0 ]];then
...
acme(){
    ...
    ~/.acme.sh/acme.sh --issue -d jaskon139-46589.portmap.io --standalone -k ec-256 --force
    if [[ $? -eq 0 ]];then
        echo -e "${OK} ${GreenBG} SSL 证书生成成功 ${Font}"
        sleep 2
        ~/.acme.sh/acme.sh --installcert -d jaskon139-46589.portmap.io --fullchainpath /home/wwwroot/ssl/h5ai.crt --keypath /home/wwwroot/ssl/h5ai.key --ecc
        if [[ $? -eq 0 ]];then
...
h5ai_install(){
    mkdir -p /home/wwwroot/jaskon139-46589.portmap.io && cd /home/wwwroot/jaskon139-46589.portmap.io&& wget ${h5ai_download_http} && unzip ${h5ai_zip}
    if [[ $? -eq 0 ]];then
...
h5ai_dependency(){
    ...
    chmod 666 /home/wwwroot/jaskon139-46589.portmap.io/_h5ai/private/cache
    echo -e "${OK} ${GreenBG} 成功添加 Public Cache directory 支持 ${Font}"
    sleep 1
    chmod 666 /home/wwwroot/jaskon139-46589.portmap.io/_h5ai/public/cache
...
domain_check(){
    ...
    domain_ip=`ping jaskon139-46589.portmap.io -c 1 | sed '1{s/[^(]*(//;s/).*//;q}'`
